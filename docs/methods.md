...
Unlike MGnify input (which always arrives with predicted proteins), third-party records may only supply a nucleotide sequence. Before such records can go through protein-based annotation (ViPhOGs taxonomy, AMR detection, functional annotation), genes are predicted with **[Pyrodigal](https://github.com/althonos/pyrodigal)**, a Python/Cython reimplementation of Prodigal used here in metagenomic mode. Input FASTA files are first decompressed and validated (invalid records are logged separately rather than silently dropped), then run through Pyrodigal to produce a GFF of predicted CDS features and the corresponding translated protein FASTA, so that from this point onward third-party and MGnify records carry the same `gff`/`faa` pair.
