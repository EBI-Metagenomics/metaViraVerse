...
- **Deduplication priority** — when the same underlying sequence shows up from both an MGnify and a third-party source, the MGnify record always wins, regardless of the `genome`/`metagenome` value on either side; the `genome` vs. `metagenome` preference only breaks ties *within* a single origin (i.e. it decides between two MGnify records, or between two third-party records, but an MGnify `genome` record still outranks a third-party `metagenome` one). Biome and other metadata from every contributing record are merged into the final entry regardless of which one wins.
